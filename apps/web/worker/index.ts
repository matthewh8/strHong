...
self.addEventListener('push', (event) => {
  ...
  event.waitUntil(
    self.registration.showNotification(data.title ?? 'Hydration Reminder', {
...
self.addEventListener('notificationclick', (event) => {
  ...
  const url: string = (event.notification.data as { url?: string })?.url ?? '/hydration';
  event.waitUntil(
    self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then((clients) => {
      const existing = clients.find((c) => c.url.includes('/hydration'));
